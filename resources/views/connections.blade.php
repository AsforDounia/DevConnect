        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-12">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div
                        class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white shadow-lg transform hover:scale-105 transition-all">
                    ...
                    <div
                        class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-2xl p-6 text-white shadow-lg transform hover:scale-105 transition-all">
                    ...
                    <div
                        class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl p-6 text-white shadow-lg transform hover:scale-105 transition-all">
                        <div class="flex items-center">
                            <div class="p-3 bg-white/20 rounded-lg backdrop-blur-sm">
                                <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div
                        class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white shadow-lg transform hover:scale-105 transition-all">
                        <div class="flex items-center">
                            <div class="p-3 bg-white/20 rounded-lg backdrop-blur-sm">
                                <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @foreach ($pandingRequests->take(2) as $request)
                            <div class="p-6 hover:bg-gray-50 transition-all duration-300">
                                ...
                                <div class="flex space-x-3 mt-4">
                                    <form class="w-1/2" action="{{ route('connections.accept', $request->sender->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('POST')
                                        <button type="submit"
                                            class="w-full px-4 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all duration-300 font-medium">Accept</button>
                                    </form>
                                    <form class="w-1/2" action="{{ route('connections.ignore', $request->sender->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('POST')
                                        <button type="submit"
                                            class="w-full px-4 py-2.5 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition-all duration-300 font-medium">Ignore</button>
                                    </form>
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                    ...
                    <div class="divide-y divide-gray-100 overflow-y-auto" style="height: 300px;">
                        @foreach ($connections as $connection)
                            <div class="p-6 hover:bg-gray-50 transition-all duration-300">
                                <div class="flex items-center space-x-4">
                                    <img src="{{ Storage::url($connection->image) ?? 'https://ui-avatars.com/api/?name=' . urlencode($connection->name) }}" alt="{{ $connection->name }}" class="w-14 h-14 rounded-full border-4 border-green-100 shadow-sm">
                                    <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if ($suggestion->status == 'pending')
                                            <button
                                                class="mt-3 w-full px-4 py-2.5 bg-gray-100 text-gray-600 rounded-xl transition-all duration-300 font-medium">
                                                <i class="mr-2 fa-solid fa-hourglass-start"></i>Pending
                                            </button>
                                        @else
                                            <form action="{{ route('connections.send', $suggestion->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('POST')
                                                <button type="submit"
                                                    class="mt-3 w-full px-4 py-2.5 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all duration-300 font-medium">
                                                    Connect
                                                </button>
                                            </form>
                                        @endif
