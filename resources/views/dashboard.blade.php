            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div x-show="showModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                        ...
                        <form action="/posts" method="POST" enctype="multipart/form-data" class="mt-4">
                            ...
                            <div class="mb-4">
                                <label class="block text-gray-700">Add the <span x-text="contentType"></span></label>
                    ...
                    <div class="p-4">
                        <div class="flex items-center justify-between">
                           ...
                           <div class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click="open = false" class="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-20">
                                    <button data-id="{{$post  -> id}}" data-content="{{$post -> content}}" class="edit-post block px-4 py-2 text-gray-700 hover:bg-gray-100 w-full">Edit Post</button>
                                    <form action="/posts/{{ $post -> id }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class=" block px-4 py-2 text-rose-700 hover:bg-gray-100 transition-colors duration-200 w-full">Delete Post</button>
                                    </form>
                ...
                <div id="editPostModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
                    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                        ...
                        <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4 z-50">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="post_id" value="{{ $post->id }}">

                            <div>
                                <textarea name="content" placeholder="Edit your post..." class="w-full border border-gray-300 rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200">{{ old('content', $post->content) }}</textarea>
                            </div>

                            @if(old('content_type', $post->post_type) === 'image')
                            <div>
                                <label class="block text-gray-700 mb-2">Current Image</label>
                                @if($post->post_type === 'image' && $post->content_type)
                                    <img src="{{ Storage::url($post->content_type) }}" class="w-full h-auto max-h-48 object-cover rounded-lg mb-4">
                                @endif
                                <input type="file" name="image" class="w-full border border-gray-300 rounded-lg p-2 text-sm">
                            </div>
                            @endif

                            @if(old('content_type', $post->post_type) === 'link')
                            <div>
                                <input type="text" name="link" placeholder="Add a link..."
                                       value="{{ old('link', $post->post_type === 'link' ? $post->content_type : '') }}"
                                       class="w-full border border-gray-300 rounded-lg p-2 text-sm">
                            </div>
                            @endif

                            @if(old('content_type', $post->post_type) === 'code')
                            <div>
                                <textarea name="code" placeholder="Add code snippet..." rows="7"
                                          class="text-white bg-black w-full border border-gray-300 rounded-lg p-2 text-sm">{{ old('code', $post->post_type === 'code' ? $post->content_type : '') }}</textarea>
                            </div>
                            @endif

                            <div class="my-4">
                                <input type="text" name="hashtags"
                                       value="{{ old('hashtags', $post->tags->pluck('name')->implode(',')) }}"
                                       class="w-full p-4 border rounded-xl focus:ring-blue-500 focus:border-blue-500"
...
<script>
    ...
    search(){
        ...
        window.location.href = `${searchRoute}?query=${encodeURIComponent(query)}`;
    }
    };
</script>
@livewireScripts
